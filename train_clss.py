...
from utils.clss_utils.clip_adapter_model import clip_adapter_model
from utils.clss_utils.SUEM_model import SUEM_model
...
def train(train_loader, test_loader, optimizer, scheduler, device, ngpus, nepoches, log_interval, log_dir, checkpoint_interval, SUEM_model):  # ---------
    ...
    for epoch in range(nepoches):
        if epoch % checkpoint_interval == 0:  # save model
            print('='*40)
            if epoch % checkpoint_interval == 0 and epoch > 200:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.0007, help='Initial learing rate')
    parser.add_argument('--decay_rate', type=float, default=1e-4, help='Initial learing rate')
    parser.add_argument('--nepoches', type=int, default=401, help='Number of traing epoches')
    ...
    clip_adapter_model = clip_adapter_model(clip_model, preprocess, device)
    clip_adapter_model = clip_adapter_model.to(device)
    clip_adapter_model = clip_adapter_model.float()

    loss = cls_loss().to(device)

    SUEM_model = SUEM_model(clip_adapter_model, model, loss, device)  #model
    # try:
    #     SUEM_model.load_state_dict(torch.load(''))
    #     SUEM_model.eval()
    #     print("导入模型成功")
